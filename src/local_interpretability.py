...
def lime_interpreter(X_train):
    lime_explainer = LimeTabularExplainer(X_train.values, feature_names=X_train.columns, discretize_continuous=True, mode='classification', categorical_features=np.where(X_train.nunique() <= 2)[0].tolist(), discretizer='decile')
    return lime_explainer

def lime_interpretation(lime_explainer, model, instance, show=True, filename=None):
    lime_explanation = lime_explainer.explain_instance(instance.values, model.predict_proba, num_features=10)
    if show:
